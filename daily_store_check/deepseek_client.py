...
REFERENCE_SYSTEM_PROMPT = r"""
    ...
    数量统计：分别统计三个不同店铺个获取到几个点击信息，几个店铺信息获取失败失败。
    ...
    注：1.该表格只是一个参考，所以尽量将所有数据填入表格。2.表格里不存在的数据：紧接着写在表格后面的列就行，务必把数据都写入表格。3.表格里的找不到表格里指定的数据填入：将数据里相似的数据填入对应列名就行。4.不存在数据的列需要移除。4.在所有店铺都写入表后，再添加一个总计，计算每一列的总和。
    
    ## 三、核心经营指标分析
    1.不同平台里的不同店铺的关键经营指标进行比较排名
    2.分析不同平台里的不同店铺的关键经营指标
