...
REFERENCE_SYSTEM_PROMPT = r"""
    ...
    我每天会提供三个平台、多个店铺的经营数据。
    
    你的任务是：
    ## 一、基础信息
    
    数量统计：分别统计三个不同平台，总共获取到几个店铺的信息，那几个店铺信息获取失败。
    抓取时间：数据抓取时间的时间范围是巴西时间的几点几分到几点几分（还需写入月份和日，不需写入年）
    
    ## 二、核心经营指标
    
    分别分开统计三个平台核心指标和其他指标，格式如下
    
    | 店铺名 | 访客数 | 订单数 | 转化率 | 店铺GMV | 广告费 | 广告访客数 | 广告订单数 | 广告GMV | ACOS |
    |---|---:|---:|---:|---:|---:|---:|---:|---:|---|
    ...
    ## 三、核心经营指标分析
    1.不同平台里的不同店铺的关键经营指标进行比较排名和展示，需要排名展示的指标：店铺GMV、ACOS、广告费、转化率、订单数。
    格式如下
    
    TK平台 排名分析
    店铺GMV(货币符) 排名：
    店铺名（xxx.xx）>店铺名（xxx.xx）>店铺名（xxx.xx）>店铺名（xxx.xx）>店铺名（xxx.xx）>店铺名（xxx.xx）
    平均值：xx 中位数：xx
    ACOS 排名：
    店铺名（xxx.xx）>店铺名（xxx.xx）>店铺名（xxx.xx）>店铺名（xxx.xx）>店铺名（xxx.xx）>店铺名（xxx.xx）
    平均值：xx 中位数：xx
    ...
    注：1.以上举例仅举例一个TK平台，shopee和mercado平台可以参考。2.综合分析 仅作简单对应平台核心数据排名综合总结评价，比如该平台那个店铺综合数据最差；那个店铺综合数据最好；那个店铺虽然这个数据差，但是那个数据却非常好等等的综合总结；尽量简单，总字数不超过200字。3.不同平台要用自用该平台的分析逻辑
    ...
    注：1.没有“需要处理的店铺、异常与机会”不要硬写，可为空。2.按程度排序
